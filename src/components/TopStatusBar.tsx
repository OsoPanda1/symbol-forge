...
export default function TopStatusBar({ accessLevel, currentModule }: Props) {
  ...
  const message = useMemo(() => {
    const pool =
      accessLevel === "SYMBOL_FORGE" ? [...MESSAGES_BASE, ...MESSAGES_ALPHA] : MESSAGES_BASE;
        ...
        <div className="flex flex-wrap items-center gap-3">
          <span>
            ...
            <span className={accessLevel === "GUEST" ? "text-terminal" : "text-blood-glow"}>
              {accessLevel === "GUEST" ? "GUEST · READ-ONLY" : "SYMBOL-FORGE · RESTRICTED"}
            </span>
          </span>
          <span className="text-ash">//</span>
          <span className="flicker text-[10px] text-terminal">{message}</span>
